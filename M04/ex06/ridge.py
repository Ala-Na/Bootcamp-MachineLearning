...
class MyRidge(MyLR):
	...
	def gradient_(self, x, y):
		if not isinstance(y, np.ndarray) or not np.issubdtype(y.dtype, np.number) or y.ndim != 2 or y.size == 0 or y.shape[1] != 1:
			return None
		if not isinstance(x, np.ndarray) or not np.issubdtype(x.dtype, np.number) or x.ndim != 2 or x.size == 0 or x.shape[0] != y.shape[0] or x.shape[1] != self.theta.shape[0] - 1:
			return None
		try:
			m = y.shape[0]

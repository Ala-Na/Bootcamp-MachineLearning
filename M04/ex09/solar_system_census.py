...
def generate_others_polynomial_forms(max_degree, to_avoid, x, x_test):
	forms = []
	x_poly = []
	x_test_poly = []
	...
	return forms, x_poly, x_test_poly
...
def draw_comparison_scatter_plot(x, y, y_hat):
	...
	for i, pair in enumerate(itertools.combinations(['weight','height','bone_density'], 2)):
		...
		# Draw Venus spots
		ax.scatter(x[pair[0]].loc[(y_hat == 0)], x[pair[1]].loc[(y_hat == 0)], 15, label=labels_pred[0], color=colors[0], zorder=1)
		ax.scatter(x[pair[0]].loc[(y == 0)], x[pair[1]].loc[(y == 0)], 15, label=labels_true[0], facecolors='none', edgecolors=colors[0], zorder=2)
		# Draw Earth spots
		ax.scatter(x[pair[0]].loc[(y_hat == 1)], x[pair[1]].loc[(y_hat == 1)], 15, label=labels_pred[1], color=colors[1], zorder=1)
		ax.scatter(x[pair[0]].loc[(y == 1)], x[pair[1]].loc[(y == 1)], 15, label=labels_true[1], facecolors='none', edgecolors=colors[1], zorder=2)
		# Draw Mars spots
		ax.scatter(x[pair[0]].loc[(y_hat == 2)], x[pair[1]].loc[(y_hat == 2)], 15, label=labels_pred[2], color=colors[2], zorder=1)
		ax.scatter(x[pair[0]].loc[(y == 2)], x[pair[1]].loc[(y == 2)], 15, label=labels_true[2], facecolors='none', edgecolors=colors[2], zorder=2)
		# Draw Belt spots
		ax.scatter(x[pair[0]].loc[(y_hat == 3)], x[pair[1]].loc[(y_hat == 3)], 15, label=labels_pred[3], color=colors[3], zorder=1)
		ax.scatter(x[pair[0]].loc[(y == 3)], x[pair[1]].loc[(y == 3)], 15, label=labels_true[3], facecolors='none', edgecolors=colors[3], zorder=2)
...
if __name__ == '__main__':
	...
	if os.path.isfile('./models.csv'):
		...
		best_form = launch_benchmark(60000, 0.0005)
	...
	for lambda_ in lambdas:
		score, _, y_hat = train_model(100000, form, x_train, y_train, x_test, y_test, lambda_, 0.0005, on_test=True)
		y_hats.append(y_hat)
		scores.append(score)
	...
	## Calculate and display F1 score for all models on testing set
	#print('\n\033[92mF1 score for all others models on testing set:\033[0m')
	## Generate polynomial forms with maximum degree of 3 and avoiding current form
	#forms, x_poly, x_test_poly = generate_others_polynomial_forms(3, form, x_train, x_test)
	
	## Train differents models with differents lambdas
	#for form in range(len(forms)):
	#	for lambda_ in lambdas:
	#		train_model(60000, forms[form], x_poly[form], y_train, x_test_poly[form], y_test, lambda_, 0.0005, on_test=True)
